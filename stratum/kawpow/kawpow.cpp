...
void get_nonce_prefix(char* extraonce1)
{
    ...
    if (g_twobyte_extranonce) {
        // 2-byte prefix (4 hex chars). Keep the high bit set so the prefix is
        // non-zero and stays in a consistent range (0x8000-0xffff).
        // Using %hhx truncates the value to 1 byte and breaks uniqueness.
        unsigned int v = ((unsigned int)g_extraonce1_counter & 0x7fffU) | 0x8000U;
        sprintf(extraonce1, "%04x", v);
    } else {
        unsigned int v = ((unsigned int)g_extraonce1_counter & 0x7fffffU) | 0x800000U;
        sprintf(extraonce1, "%06x", v);
    }
