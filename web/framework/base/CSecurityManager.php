...
class CSecurityManager extends CApplicationComponent
{
	...
	public function encrypt($data,$key=null)
	{
		...
		// PHP 7.2+ removed mcrypt. Prefer OpenSSL (available by default on modern PHP builds).
		// We keep the output format compatible with the legacy implementation: IV + raw ciphertext.
		if(function_exists('openssl_encrypt'))
		{
			...
			// Normalize key length for AES-256 using SHA-256 (binary)
			$derivedKey = hash('sha256', $key, true);
			$encrypted = openssl_encrypt($data, $cipher, $derivedKey, OPENSSL_RAW_DATA, $iv);
			if($encrypted === false)
				throw new CException(Yii::t('yii','Failed to encrypt data.'));
			return $iv.$encrypted;
		}

		// Fallback to legacy mcrypt when present (older environments).
	...
	public function decrypt($data,$key=null)
	{
		...
		if(function_exists('openssl_decrypt'))
		{
			...
			$ciphertext = $this->substr($data, $ivLen, $this->strlen($data));

			$derivedKey = hash('sha256', $key, true);
			$decrypted = openssl_decrypt($ciphertext, $cipher, $derivedKey, OPENSSL_RAW_DATA, $iv);
			if($decrypted === false)
				throw new CException(Yii::t('yii','Failed to decrypt data.'));
			return $decrypted;
		}
	...
	protected function openCryptModule()
	{
		if(extension_loaded('mcrypt'))
		{
			...
			throw new CException(Yii::t('yii','CSecurityManager requires either OpenSSL or mcrypt to be loaded in order to use data encryption feature.'));
	}
	...
	public function generateRandomBytes($length,$cryptographicallyStrong=true)
	{
		...
		// mcrypt is removed in modern PHP versions; don't reference its constants unless loaded.
		if(extension_loaded('mcrypt') && function_exists('mcrypt_create_iv') && defined('MCRYPT_DEV_URANDOM') &&
