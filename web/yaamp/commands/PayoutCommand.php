...
class PayoutCommand extends CConsoleCommand
{
	...
	public function checkPayouts($symbol, $fixit)
	{
		...
		foreach ($ids as $uid => $user_addr)
		{
			...
			echo "$user_addr payouts since ".date('Y-m-d H:i:s', (int) $since).": ".count($payouts)."\n";
			...
			if ($totaldiff > 0.0) {
				...
				foreach ($payouts as $payout) {
					$time = date('Y-m-d H:i:s', (int) $payout->time);
	...
	protected function checkPayoutsConfirmations($args)
	{
		...
		foreach ($data as $row) {
			...
			echo date('Y-m-d H:i', (int) $row['time'])." $txid ".$tx['confirmations'].
