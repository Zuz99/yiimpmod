...
if (!empty($txs))
{
    ...
    $tx = reset($txs);
	if (count($txs) == $maxrows) $lastday = date('Y-m-d', (int) arraySafeVal($tx, 'blocktime', $tx['time']));
...
if (!empty($txs_array))
{
    ...
    foreach ($txs_array as $key => $tx)
    {
		...
		if ($lastday == '' && count($txs) == $maxrows) $lastday = date('Y-m-d', (int) arraySafeVal($tx, 'blocktime', $tx['time']));
    }
